...
PERMITTED_MODEL_INTENTS = {
    ...
    "stage-a-blocked-insufficient-source",
...
def _result(category, permitted, allowlist_shape, reason_code):
    return {
        ...
        "prohibited_scopes": list(STAGE_A_PROHIBITED_SCOPES)
        if category.startswith("stage-a-") else [],
...
def validate_model_intent(text, model_intent):
    ...
    if model_intent == "stage-a-inbox-only":
        if not has_url:
            return _result("stage-a-blocked-insufficient-source", False, "none",
                           "model-approved-stage-a-without-url")
        return _result("stage-a-inbox-only", True, "single-new-inbox-markdown",
                       "model-semantic-inbox-decision")
    if model_intent == "stage-a-inbox-only-lightweight":
        if not normalized:
            return _result("stage-a-blocked-insufficient-source", False, "none",
                           "model-approved-lightweight-without-payload")
        return _result("stage-a-inbox-only-lightweight", True,
                       "single-new-inbox-markdown", "model-semantic-inbox-decision")
    return _result(model_intent, False,
                   "formal-flow-required" if model_intent == "formal-promotion" else "none",
                   "model-semantic-non-stage-a-decision")
...
def main(argv=None):
    ...
    parser.add_argument("--model-intent", required=True,
